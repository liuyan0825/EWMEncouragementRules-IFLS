...
theta2 = (W2'*W2)\(W2'*Y);
beta2 = theta2(27:52)-theta2(1:26);
alpha2 = theta2(53);

% 3-th order polynomial
W3 = [X0 Z20 X1 Z21 p.^2-p p.^3-p];
theta3 = (W3'*W3)\(W3'*Y);
beta3 = theta3(27:52)-theta3(1:26);
alpha3 = theta3(53:54);

% 4-th order polynomial
W4 = [X0 Z20 X1 Z21 p.^2-p p.^3-p p.^4-p];
theta4 = (W4'*W4)\(W4'*Y);
beta4 = theta4(27:52)-theta3(1:26);
alpha4 = theta4(53:55);

% 5-th order polynomial
W5 = [X0 Z20 X1 Z21 p.^2-p p.^3-p p.^4-p p.^5-p];
theta5 = (W5'*W5)\(W5'*Y);
beta5 = theta5(27:52)-theta5(1:26);
alpha5 = theta5(53:56);

% 6-th order polynomial
W6 = [X0 Z20 X1 Z21 p.^2-p p.^3-p p.^4-p p.^5-p p.^6-p];
theta6 = (W6'*W6)\(W6'*Y);
beta6 = theta6(27:52)-theta6(1:26);
alpha6 = theta6(53:57);
...
u6 = [2*u 3*u.^2 4*u.^3 5*u.^4 6*u.^5]-1;
MTE6 = [ones(100,1) Xbar Z2bar]*beta6+u6*alpha6;
beta = betae(26:50)-betae(1:25);
MTElqr = [Xbar Z2bar]*beta+lqr;

h = figure('Color','white');
plot(u,MTE2,'--','LineWidth',1.2,'Color',[0.6350 0.0780 0.1840])
hold on
plot(u,MTE3,':','LineWidth',1.2,'Color',[0.9290 0.6940 0.1250])
plot(u,MTE4,'-.','LineWidth',1.2,'Color',[0.4940 0.1840 0.5560])
plot(u,MTE5,'--','LineWidth',1.2,'Color',[0 0.4470 0.7410])
plot(u,MTE6,':','LineWidth',1.2,'Color',[0.4660 0.6740 0.1880])
plot(u,MTElqr,'-','LineWidth',1.2)
legend('2-th order','3-th order','4-th order','5-th order','6-th order', ...
    'local quadratic','Location','southwest')
xlabel('$u_1$','interpreter','latex');
ylabel('$\widehat{MTE}_1(u_1,\bar{X},\bar{Z}_2)$','interpreter','latex');
